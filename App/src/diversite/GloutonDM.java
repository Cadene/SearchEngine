...
public class GloutonDM extends IRModel {

	private IRModel baseIRModel;
	private int nbDocs;
	private double alpha;

	public GloutonDM(Weighter weighter, IRModel baseIRModel, int nbDocs, double alpha) {
		super(weighter);
	...
	protected void processScores(HashMap<String, Integer> query) throws Exception {
		...
		double j = this.nbDocs;
		for (int i = 0; i < this.nbDocs; i++){
	...
	private double sim_psi(HashMap<String,Double> doc1, HashMap<String, HashMap<String,Double>> R) {
		...
		for(HashMap<String,Double> doc2 : R.values()) {
			if (doc2.isEmpty() == false){
				rslt += 1-similarity(doc1, doc2);
			}
		}
		return rslt / R.size();
	}
	
	private double similarity(HashMap<String,Double> doc1, HashMap<String,Double> doc2) {
		double rslt = 0.0;
		double norm1 = 0.0;
		double norm2 = 0.0;
